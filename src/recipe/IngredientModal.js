...
function IngredientModal(props) {
      ...
      return (
        <div style={{ display: "flex", justifyContent: "space-around" }}>
          ...
          <Form.Control
            ...
            type="number"
            placeholder={ingredient.quantity || "Quantity"}
          />
          <Form.Select
            key={ingredient}
            onChange={(e) => updateMeasurement(e, ingredient)}
  ...
  const updateMeasurement = (e, ingredient) => {
    const targetIndex = props.ingredients.findIndex(
      (ing) => ing === ingredient
    );
  ...
  const addIngredient = (e) => {
    ...
    for (let i = 0; i < ingredients.length - 3; i += 3)
      newList.push({
        ...
        measurement: ingredients[i + 2].value,
      });
